...
<?php
echo "<tr><td>กราฟแสดงจำนวนความเสี่ยงแยกตามระบบ</font></td></tr><br><br>";
echo "<tr><td>ช่วงวันที่  $startdate  ถึง   $enddate</td></tr><br><br>";
	...
	$strQuery = "SELECT c.source_code_name,count(c.source_code_id) as count_source_code_id
FROM
register_risk r
...
where r.`date` between '$startdate'and'$enddate' and c.source_code_id <> '0' group by c.source_code_id
	...
	if ($result) {
		while($ors = mysql_fetch_array($result)) {
			...
			$strXML .= "<set label='" . $ors['source_code_name'] . "' value='" . $ors['count_source_code_id'] . "' />";
