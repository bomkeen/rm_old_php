...
echo "<tr><td>ช่วงวันที่  $startdate  ถึง   $enddate</td></tr><br><br>";
	...
	//$a2 = date("Y");
	$graph = "Pie2D";
	$strXML = "<chart caption='กราฟจำนวนด้านความเสี่ยงแยกตามประเภท  ช่วงวันที่  $startdate  ถึง   $enddate  ' subCaption='' pieSliceDepth='30' showBorder='1' formatNumberScale='0' numberSuffix=' ครั้ง'>";

	// Fetch all factory records
	$strQuery = "SELECT c.clinic_name,count(c.clinic_id) as count_clinic,r.clinic_id
FROM
register_risk r
left outer join clinic_risk c on c.clinic_id = r.clinic_id
left outer join tb_member p on p.id=r.person_id
where r.`date` between '$startdate' and '$enddate'   and r.clinic_id <> 0 group by c.clinic_id
        ";
	...
	if ($result) {
		while($ors = mysql_fetch_array($result)) {
			...
			$strXML .= "<set label='" . $ors['clinic_name'] . "' value='" . $ors['count_clinic'] . "' />";
