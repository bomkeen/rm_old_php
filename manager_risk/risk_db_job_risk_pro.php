...
<?php
echo "<tr><td>กราฟแสดงจำนวนตามโปรแกรมหลัก</font></td></tr><br><br>";
echo "<tr><td>ช่วงวันที่  $startdate  ถึง   $enddate</td></tr><br><br>";
	...
	$strQuery = "SELECT pr.pro_risk_name,count(pr.pro_risk_id) as count_pro_risk,pr.pro_risk_id
FROM
register_risk r
...
where r.`date` between '$startdate'and'$enddate'  and pr.pro_risk_id <> 0 group by pr.pro_risk_id
	...
	if ($result) {
		while($ors = mysql_fetch_array($result)) {
			...
			$strXML .= "<set label='" . $ors['pro_risk_name'] . "' value='" . $ors['count_pro_risk'] . "' />";
