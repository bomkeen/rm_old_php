...
<CENTER>
<FONT SIZE="3" COLOR="#9900CC">จำนวนความเสี่ยงแยกตามแผนกที่เกิดในปีปัจจุบัน</FONT>
<!-- php chart  -->
<?php
...
LEFT JOIN code_complaint AS c ON c.ward_id=r.wardedit_id WHERE year(r.date)=YEAR(NOW()) GROUP BY r.wardedit_id";
$suma = array();
//echo $sql;
...
$strXML .= "</chart>"; //// นำ XML มาต่อสตริง ปิด chart  ต้องเว้นวรรค หนึ่งเคาะ แล้วถึงจะ .=
	echo renderChart("../Charts/Pie3D.swf", "", $strXML, "รายงานจำนวนประชากร", 300, 150, false, false);
	?>
