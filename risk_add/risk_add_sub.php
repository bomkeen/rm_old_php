...
<FORM ACTION="risk_add_type.php" METHOD="post" ENCTYPE="multipart/form-data">
    <TABLE class="table" WIDTH="800" BORDER="0" CELLSPACING="1" CELLPADDING="0">
      ...
      <TD>รยาละเอียดหมวด</TD>
      <TD>
	  <SELECT NAME="prorisk_sub_detail_id">
	  <OPTION VALUE="0">รายละเอียดหมวด</OPTION>
	  <?php
		 include "../function/connect.php";
		$sql="select * from prorisk_sub_detail 
         where prorisk_detail_id='$prorisk_detail_id' ";
		$result=mysql_db_query($dbname,$sql);
	  	while($rs=mysql_fetch_array($result)) {
			$prorisk_sub_detail_id=$rs['id'];
			$prorisk_sub_detail_name=$rs['prorisk_sub_detail_name'];
			//echo "<OPTION VALUE='$prorisk_sub_detail_id'>$prorisk_sub_detail_name</OPTION>";
                                                
                         if ($prorisk_sub_detail_id == $prorisk_sub_detail_id2) {
        echo "<OPTION VALUE='$prorisk_sub_detail_id' SELECTED>$prorisk_sub_detail_name</OPTION>";
    } else {
        echo "<OPTION VALUE='$prorisk_sub_detail_id'>$prorisk_sub_detail_name</OPTION>";
    }
		}
